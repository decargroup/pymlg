...
class SO2(MatrixLieGroup):
    ...
    def left_jacobian(phi):
        ...
        return (s / phi) * np.identity(2) + ((1 - c) / phi) * SO2.wedge(1.)
    ...
    def left_jacobian_inv(phi):
        ...
        half_angle = 0.5 * phi
        cot_half_angle = 1. / tan(half_angle)
        return half_angle * cot_half_angle * np.identity(2) - \
            half_angle * SO2.wedge(1.)